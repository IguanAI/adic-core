...
mod tests {
    ...
    fn test_compute_trust() {
        ...
        // Trust should be ln(1 + reputation)
        assert!((trust1 - (2.0_f64).ln()).abs() < 0.001);
        assert!((trust2 - (3.0_f64).ln()).abs() < 0.001);
    }
    ...
    fn test_compute_detailed_weights() {
        ...
        // Total should be product of individual weights
        let expected_total = weights.proximity * weights.trust * weights.conflict_penalty;
    ...
    fn test_edge_cases() {
        ...
        // With all parameters 0, weight should be e^0 = 1.0
        let weight = calc.compute_weight(10.0, 10.0, 10.0);
        assert_eq!(weight, 1.0);
        
        let detailed = calc.compute_detailed_weights(10.0, 10.0, 10.0);
        assert_eq!(detailed.proximity, 1.0);
        assert_eq!(detailed.trust, 1.0);

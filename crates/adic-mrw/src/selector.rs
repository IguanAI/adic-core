use adic_types::{MessageId, AdicParams, AdicFeatures, QpDigits, AxisId, Result, AdicError};
...
impl MrwSelector {
    ...
    pub async fn select_parents(
        ...
        while attempts < MAX_ATTEMPTS {
            ...
            let candidates = self.run_mrw_per_axis(
                message_features,
                &tips,
                weight_calc,
                horizon,
    ...
    fn run_mrw_per_axis(
        ...
        for axis_idx in 0..self.params.d {
            ...
            let msg_axis = message_features.get_axis(axis_id)
            ...
            for tip in tips.iter().take(horizon) {
                ...
                let proximity = proximity_score(&msg_axis.qp_digits, tip_phi, self.params.rho[axis_idx as usize]);
    ...
    fn merge_axis_candidates(
        ...
        for (axis_id, candidates) in axis_candidates {
            for candidate in candidates {
                merged.entry(candidate.message_id)
                    .and_modify(|c| {
                    ...
                    .or_insert_with(|| {
                        ...
                        new_candidate.axis_weights.insert(*axis_id, candidate.weight);
                        new_candidate
                    });
            }
        }

...
pub struct MemoryStorage {
    balances: Arc<RwLock<HashMap<AccountAddress, AdicAmount>>>,
    locked_balances: Arc<RwLock<HashMap<AccountAddress, AdicAmount>>>,
    transaction_backup: Arc<RwLock<Option<(
        HashMap<AccountAddress, AdicAmount>,
        HashMap<AccountAddress, AdicAmount>,
    )>>>,
}
...
impl EconomicsStorage for RocksDbStorage {
    ...
    async fn get_locked_balance(&self, address: AccountAddress) -> Result<AdicAmount> {
        let cf = self.db.cf_handle(&self.cf_locked)
            .ok_or_else(|| anyhow::anyhow!("Column family not found"))?;
    ...
    async fn get_all_accounts(&self) -> Result<Vec<AccountAddress>> {
        ...
        let cf_balances = self.db.cf_handle(&self.cf_balances)
            .ok_or_else(|| anyhow::anyhow!("Column family not found"))?;
        ...
        let cf_locked = self.db.cf_handle(&self.cf_locked)
            .ok_or_else(|| anyhow::anyhow!("Column family not found"))?;
...
mod tests {
    ...
    async fn test_transaction_rollback() {
        ...
        // Modify balance
        storage.set_balance(addr, AdicAmount::from_adic(200.0)).await.unwrap();
        assert_eq!(storage.get_balance(addr).await.unwrap(), AdicAmount::from_adic(200.0));

...
async fn test_core_supply_invariants() {
    ...
    assert!(economics
        .supply
        .mint_emission(AdicAmount::from_base_units(1))
        .await
        .is_err());
...
async fn test_genesis_allocation_invariants() {
    ...
    let treasury_initial = economics
        .balances
    ...
    let treasury = economics
        .balances
        .get_balance(AccountAddress::treasury())
        .await
        .unwrap();
    let liquidity = economics
        .balances
        .get_balance(AccountAddress::liquidity_pool())
        .await
        .unwrap();
    let genesis = economics
        .balances
    ...
    let genesis_before_transfer = economics
        .balances
        .get_balance(AccountAddress::genesis_pool())
        .await
        .unwrap();
    economics
        .balances
        .transfer(AccountAddress::genesis_pool(), user, release_amount)
        .await
        .unwrap();
    let genesis_after_transfer = economics
        .balances
        .get_balance(AccountAddress::genesis_pool())
        .await
        .unwrap();
    ...
    assert_eq!(
        genesis_before_transfer,
        genesis_after_transfer.saturating_add(release_amount)
    );
...
async fn test_treasury_multisig_invariants() {
    ...
    assert!(economics
        .treasury
        .approve_proposal(proposal_id, signers[2])
        .await
        .is_err());
    ...
    let proposal_id2 = economics
        .treasury
        .propose_transfer(recipient, transfer_amount, "Test2".to_string(), signers[0])
        .await
        .unwrap();
    ...
    assert!(economics
        .treasury
        .approve_proposal(proposal_id2, signers[1])
        .await
        .unwrap());
    ...
    assert_eq!(
        treasury_after,
        treasury_before.saturating_sub(transfer_amount)
    );
...
async fn test_conservation_laws() {
    ...
    economics
        .balances
        .lock(lock_account, AdicAmount::from_adic(500.0))
        .await
        .unwrap();
    ...
    for _ in 0..10 {
        ...
        let _ = economics
            .balances
            .lock(test_accounts[1], AdicAmount::from_adic(1.0))
            .await;
        let _ = economics
            .balances
            .unlock(test_accounts[1], AdicAmount::from_adic(1.0))
            .await;
    }
...
async fn test_overflow_underflow_protection() {
    ...
    assert!(economics
        .balances
        .transfer(account1, account2, AdicAmount::from_adic(101.0))
        .await
        .is_err());
    ...
    assert!(economics
        .balances
        .lock(account1, balance.saturating_add(AdicAmount::from_adic(1.0)))
        .await
        .is_err());
    ...
    economics
        .balances
        .lock(account1, AdicAmount::from_adic(50.0))
        .await
        .unwrap();
    assert!(economics
        .balances
        .unlock(account1, AdicAmount::from_adic(51.0))
        .await
        .is_err());

...
async fn test_double_spend_protection() {
    ...
    let handle1 = tokio::spawn(async move {
        economics_clone1
            .balances
            .transfer(attacker, victim1, amount)
            .await
    });
    ...
    let handle2 = tokio::spawn(async move {
        economics_clone2
            .balances
            .transfer(attacker, victim2, amount)
            .await
    });
    ...
    if excess > total_balance {
        assert!(economics
            .balances
            .transfer(attacker, victim1, excess)
            .await
            .is_err());
    } else {
...
async fn test_authorization_attacks() {
    ...
    assert!(economics
        .treasury
        .approve_proposal(proposal_id, attacker)
        .await
        .is_err());
    ...
    economics
        .balances
        .transfer(AccountAddress::genesis_pool(), attacker, release_amount)
        .await
        .unwrap();
...
async fn test_malicious_input_handling() {
    ...
    assert!(economics
        .balances
        .transfer(account, AccountAddress::from_bytes([2; 32]), max_amount)
        .await
        .is_err());
    ...
    assert!(economics
        .balances
        .transfer(account, account, AdicAmount::from_adic(100.0))
        .await
        .is_err());
    ...
    economics
        .balances
        .transfer(account, account2, zero)
        .await
        .unwrap();
    ...
    let id1 = economics
        .treasury
        .propose_transfer(recipient, amount, "Test".to_string(), signers[0])
        .await
        .unwrap();
...
async fn test_consensus_manipulation_protection() {
    ...
    assert!(economics
        .balances
        .lock(proposer, AdicAmount::from_adic(10.0))
        .await
        .is_err());
    ...
    economics
        .balances
        .unlock(proposer, deposit_amount)
        .await
        .unwrap();
    economics
        .balances
        .transfer(proposer, AccountAddress::treasury(), deposit_amount)
        .await
        .unwrap();
    ...
    assert_eq!(
        treasury_after,
        treasury_before.saturating_add(deposit_amount)
    );
    assert_eq!(
        proposer_after,
        proposer_before.saturating_sub(deposit_amount)
    );
...
async fn test_time_based_attacks() {
    ...
    for _ in 0..100 {
        if economics
            .balances
            .transfer(spammer, victim, AdicAmount::from_adic(0.01))
            .await
            .is_ok()
        {

...
async fn stress_test_concurrent_transfers() {
    ...
    println!("Completed {} successful transfers in {:?}", total_transfers, elapsed);
    println!("Failed transfers: {}", failed);
    println!("Throughput: {:.0} transfers/sec", total_transfers as f64 / elapsed.as_secs_f64());
...
async fn stress_test_emission_processing() {
    ...
    println!("Processed {} emission events in {:?}", validators.len(), elapsed);
    println!("Total emitted: {}", total_emitted);
    println!("Average emission time: {:?}", elapsed / validators.len() as u32);
    
    // Verify supply increased correctly
    let final_supply = economics.get_total_supply().await;
    assert_eq!(final_supply, AdicAmount::GENESIS_SUPPLY.saturating_add(total_emitted));
...
async fn stress_test_treasury_proposals() {
    ...
    for i in 0..num_proposals {
        ...
        let id = economics.treasury.propose_transfer(
            ...
            amount,
            format!("Proposal {}", i),
            signers[i % num_signers as usize],
        ).await.unwrap();
        
        proposal_ids.push(id);
    }
    
    println!("Created {} proposals in {:?}", num_proposals, create_start.elapsed());
    
    // Approve some proposals concurrently
    let approve_start = Instant::now();
    let mut handles = vec![];
    
    for (_idx, proposal_id) in proposal_ids[..10].iter().enumerate() {
        let economics_clone = economics.clone();
        ...
        let handle = tokio::spawn(async move {
            ...
            for signer_idx in 1..2 { // Only need 1 more approval since threshold is 2
                if signer_idx < signers_clone.len() {
                    let _ = economics_clone.treasury.approve_proposal(
                        pid,
                        signers_clone[signer_idx],
                    ).await;
                }
            }
        });
...
async fn stress_test_memory_pressure() {
    ...
    for i in 0..iterations {
        ...
        if i < 10 {
            ...
            let _ = economics.treasury.propose_transfer(
                account2,
...
async fn benchmark_critical_operations() {
    ...
    for _ in 0..100 {
        let start = Instant::now();
        economics.balances.transfer(account1, account2, AdicAmount::from_adic(1.0))
            .await.unwrap();
        times.push(start.elapsed());
        
        // Transfer back
        economics.balances.transfer(account2, account1, AdicAmount::from_adic(1.0))
            .await.unwrap();
    }
    ...
    for _ in 0..100 {
        ...
        economics.balances.unlock(account1, AdicAmount::from_adic(100.0)).await.unwrap();
        times.push(start.elapsed());
    }
    ...
    assert!(avg_query < Duration::from_millis(1), "Balance query too slow");
    assert!(avg_transfer < Duration::from_millis(10), "Transfer too slow");
    assert!(avg_metrics < Duration::from_millis(1), "Metrics query too slow");
...
async fn test_recovery_under_load() {
    ...
    let load_handle = tokio::spawn(async move {
        for i in 0..1000 {
            ...
            let _ = economics_clone.balances.transfer(
                from,
                to,

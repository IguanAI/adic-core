...
async fn test_complete_token_lifecycle_with_all_operations() {
    ...
    assert_eq!(treasury_balance, AdicAmount::from_adic(total_genesis.to_adic() * 0.20));
    assert_eq!(liquidity_balance, AdicAmount::from_adic(total_genesis.to_adic() * 0.30));
    assert_eq!(genesis_balance, AdicAmount::from_adic(total_genesis.to_adic() * 0.50));
    
    // Verify sum equals total
    let sum = treasury_balance.saturating_add(liquidity_balance).saturating_add(genesis_balance);
    ...
    for (threshold, total) in configs {
        ...
        for i in 1..threshold {
            ...
            let executed = economics.treasury.approve_proposal(
                proposal_id,
                signers[i as usize],
            ).await.unwrap();
    ...
    for amount in &lock_amounts {
        ...
        let locked = economics.balances.get_locked_balance(depositor).await.unwrap();
        assert_eq!(locked, total_locked);
        
        let unlocked = economics.balances.get_unlocked_balance(depositor).await.unwrap();
        assert_eq!(unlocked, initial_balance.saturating_sub(total_locked));
    }
    
    // Partial unlock
    economics.balances.unlock(depositor, lock_amounts[0]).await.unwrap();
    total_locked = total_locked.saturating_sub(lock_amounts[0]);
    
    let locked_after = economics.balances.get_locked_balance(depositor).await.unwrap();
    ...
    if excess_transfer > total_balance {
        ...
        let _result = economics.balances.transfer(
            depositor,
            test_accounts[9],
            excess_transfer,
        ).await;
    ...
    let actual_treasury = economics.balances.get_balance(AccountAddress::treasury()).await.unwrap();
...
async fn test_edge_cases_and_boundaries() {
    ...
    economics.balances.transfer(account1, account2, AdicAmount::ZERO).await.unwrap();
    ...
    economics.balances.transfer(max_account, account1, tiny_amount).await.unwrap();
    
    // Test 4: Self-transfer (should fail)
    assert!(economics.balances.transfer(account1, account1, AdicAmount::from_adic(10.0)).await.is_err());
...
async fn test_concurrent_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for j in 0..10 {
                ...
                economics_clone.balances.transfer(from, to, amount).await.unwrap();
    ...
    for i in 0..10 {
        ...
        let receiver_balance = economics.balances.get_balance(accounts[i + 10]).await.unwrap();
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            let amount = AdicAmount::from_adic(100.0 * (i + 1) as f64);
            economics_clone.balances.lock(lock_account, amount).await.unwrap();
        });
    ...
    let total_locked = economics.balances.get_locked_balance(lock_account).await.unwrap();
...
async fn test_error_recovery_and_rollback() {
    ...
    assert!(economics.balances.transfer(account1, account2, excessive_amount).await.is_err());
    ...
    assert!(economics.balances.lock(account1, excessive_amount).await.is_err());
    
    // Test 3: Unlock more than locked (should fail)
    economics.balances.lock(account1, AdicAmount::from_adic(100.0)).await.unwrap();
    assert!(economics.balances.unlock(account1, AdicAmount::from_adic(200.0)).await.is_err());
    
    // Locked amount should still be 100
    let locked = economics.balances.get_locked_balance(account1).await.unwrap();
    ...
    let balance = economics.balances.get_balance(genesis_account).await.unwrap();
...
async fn test_long_term_simulation() {
    ...
    for year in 1..=100 {
        ...
        if year % 10 == 0 {
            println!("Year {}: Supply = {:.2}M ADIC, Rate = {:.4}%", 
                     year, 
...
async fn test_maximum_load_handling() {
    ...
    for account in &accounts[..100] { // Transfer to first 100 accounts
        economics.balances.transfer(
            AccountAddress::genesis_pool(),
            *account,
            amount_per_account
    ...
    println!("Performed {} transfers in {:?}", transfer_count, transfer_time);
...
async fn test_treasury_proposal_lifecycle() {
    ...
    economics.treasury.reject_proposal(proposal_id, signers[1]).await.unwrap();
...
async fn test_system_invariants_maintained() {
    ...
    sum = sum.saturating_add(economics.balances.get_balance(AccountAddress::treasury()).await.unwrap());
    sum = sum.saturating_add(economics.balances.get_balance(AccountAddress::liquidity_pool()).await.unwrap());
    sum = sum.saturating_add(economics.balances.get_balance(AccountAddress::genesis_pool()).await.unwrap());
    ...
    for account in &accounts {
        ...
        let locked_balance = economics.balances.get_locked_balance(*account).await.unwrap();

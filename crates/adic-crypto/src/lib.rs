use adic_types::{AdicMessage, PublicKey, Signature, Result, AdicError};
...
impl Keypair {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != 32 {
            return Err(AdicError::InvalidParameter("Invalid key length".to_string()));
        }
...
impl CryptoEngine {
    ...
    pub fn sign_message(&self, message: &AdicMessage) -> Result<Signature> {
        let signing_key = self.signing_key.as_ref()
            .ok_or(AdicError::InvalidParameter("No signing key available".to_string()))?;
...
mod tests {
    use super::*;
    use adic_types::{AdicFeatures, AxisPhi, QpDigits, AdicMeta, MessageId};
    use chrono::Utc;
    ...
    fn test_sign_and_verify() {
        ...
        let valid = crypto.verify_signature(&message, &public_key, &signature).unwrap();
        assert!(valid);
        
        let wrong_key = PublicKey::from_bytes([0; 32]);
        let invalid = crypto.verify_signature(&message, &wrong_key, &signature).unwrap();
        assert!(!invalid);
    }
}
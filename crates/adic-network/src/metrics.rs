...
impl NetworkMetrics {
    pub fn new() -> Self {
        let messages_sent = register_counter!(
            "adic_messages_sent_total",
        ...
        let messages_by_type = register_counter_vec!(
            "adic_messages_by_type",
            "Messages by type",
            &["type"]
        ).unwrap();
        ...
        let connected_peers = register_gauge!(
            "adic_connected_peers",
        ...
        let bytes_sent = register_counter!(
            "adic_bytes_sent_total",
            "Total bytes sent"
        ).unwrap();
        
        let bytes_received = register_counter!(
            "adic_bytes_received_total",
            "Total bytes received"
        ).unwrap();
        
        let active_connections = register_gauge!(
            "adic_active_connections",
        ...
        let sync_progress = register_gauge!(
            "adic_sync_progress",
    ...
    pub async fn record_message_received(&self, message_id: &MessageId, message_type: &str) {
        ...
        self.messages_by_type.with_label_values(&[message_type]).inc();
        ...
        if let Some(sent_time) = timestamps.get(message_id) {
            ...
            self.message_latency.with_label_values(&[message_type]).observe(latency);
        }
    }
    ...
    pub async fn record_peer_bandwidth(&self, peer: &PeerId, upload_mbps: f64, download_mbps: f64) {
        ...
        self.peer_bandwidth.with_label_values(&[&peer_str, "upload"]).set(upload_mbps);
        self.peer_bandwidth.with_label_values(&[&peer_str, "download"]).set(download_mbps);
    ...
    pub fn record_peer_latency(&self, peer: &PeerId, latency_ms: f64) {
        let peer_str = peer.to_string();
        self.peer_latency.with_label_values(&[&peer_str]).observe(latency_ms);
    }
    ...
    pub fn record_connection_duration(&self, peer: &PeerId, duration: Duration) {
        let peer_str = peer.to_string();
        self.connection_duration.with_label_values(&[&peer_str])
            .observe(duration.as_secs_f64());
    }
    
    pub fn record_gossip_event(&self, topic: &str, action: &str) {
        self.gossip_messages.with_label_values(&[topic, action]).inc();
    }
...
mod tests {
    ...
    async fn test_message_metrics() {
        ...
        metrics.record_message_received(&message_id, "test_type").await;

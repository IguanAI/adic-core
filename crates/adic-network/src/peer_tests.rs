...
mod tests {
    ...
    use crate::peer::{ConnectionState, MessageStats, PeerEvent, PeerInfo, PeerScore};
    use adic_math::distance::padic_distance;
    use adic_types::{features::QpDigits, PublicKey};
    ...
    fn test_peer_score_default() {
        ...
        let total = score.latency_weight
            + score.bandwidth_weight
            + score.reputation_weight
            + score.uptime_weight
    ...
    async fn test_deposit_verifier_real() {
        ...
        // Test with invalid response - empty response should always fail
        assert!(!verifier.verify_pow_response(&challenge, &[]).await);

        // Test with a response that's unlikely to produce required zeros
        // but we can't guarantee it won't work due to hash randomness
        let bad_response = [0x00; 8];
        let _is_valid = verifier
            .verify_pow_response(&challenge, &bad_response)
            .await;
        // Just verify the function runs without panic, result may vary
    }
}
